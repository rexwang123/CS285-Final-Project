...
class DiffBootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        return q_values
    def update(self, ob_no1, ac_na1, next_ob_no1, reward_n1, terminal_n1, ob_no2, ac_na2, next_ob_no2, reward_n2, terminal_n2, mt = True):
        """
        ...
        for k in range(10):
            ...
            for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
                if i % self.num_grad_steps_per_target_update == 0:
                    if mt == False:
                        v = self(torch.cat((next_ob_no2_, next_ob_no1), -1))
                        v2 = self(torch.cat((next_ob_no1, next_ob_no2_), -1))
                        target = (reward_n2_ - reward_n1 + self.gamma * v * (1 - terminal_n1 * terminal_n2_)).detach()
                        target2 = (reward_n1 - reward_n2_ + self.gamma * v2 * (1 - terminal_n1 * terminal_n2_)).detach()
                        # v = self(ob_no1)
                        # target = (reward_n1 + self.gamma * v * (1-terminal_n1)).detach()
                curr = self(torch.cat((ob_no2_, ob_no1), -1))
                curr2 = self(torch.cat((ob_no1, ob_no2_), -1))
                curr3 = self(torch.cat((ob_no1, ob_no1), -1))
                # curr = self(ob_no1)
