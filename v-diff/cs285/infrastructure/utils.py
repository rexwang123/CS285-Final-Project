...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    next_rewards = rewards[1:].copy()
    next_rewards.append(0.0)
    return Path(obs, image_obs, acs, rewards, next_rewards, next_obs, terminals)
...
def Path(obs, image_obs, acs, rewards, next_rewards, next_obs, terminals):
    """
    ...
    return {"observation" : np.array(obs, dtype=np.float32),
            ...
            "next_reward" : np.array(next_rewards, dtype=np.float32),
...
def convert_listofrollouts(paths):
    ...
    concatenated_next_rewards = np.concatenate([path["next_reward"] for path in paths])
    unconcatenated_rewards = [path["reward"] for path in paths]
    unconcatenated_next_rewards = [path["next_reward"] for path in paths]
    return observations, actions, next_observations, terminals, concatenated_rewards, concatenated_next_rewards, unconcatenated_rewards, unconcatenated_next_rewards
