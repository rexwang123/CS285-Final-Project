...
class DiffACAgent(BaseAgent):
    ...
    def train(self, ob_no1, ac_na1, re_n1, next_re_n1, next_ob_no1, terminal_n1, ob_no2, ac_na2, re_n2, next_ob_no2, terminal_n2, mt = True):
        ...
        if(mt == False):
            for i in range(self.agent_params['num_critic_updates_per_agent_update']):
                loss['Critic_Loss'] += self.critic.update(ob_no1,ac_na1,next_ob_no1, re_n1,terminal_n1,ob_no2,ac_na2,next_ob_no2,re_n2, terminal_n2, self.actor, False)
        else:
            for i in range(self.agent_params['num_critic_updates_per_agent_update']):
                loss['Critic_Loss'] += self.critic.update(ob_no1,ac_na1,next_ob_no1, re_n1,terminal_n1,ob_no2,ac_na2,next_ob_no2,re_n2, terminal_n2, self.actor, True)
        
        diff = self.estimate_diff(ob_no1, ac_na1, next_ob_no1, re_n1, next_re_n1, terminal_n1)
    ...
    def estimate_diff(self, ob_no, ac_na, next_ob_no, re_n, next_re_n, terminal_n):
        ob_no = ptu.from_numpy(ob_no)
        next_ob_no = ptu.from_numpy(next_ob_no)
        terminal_n = ptu.from_numpy(terminal_n)
        re_n = ptu.from_numpy(np.concatenate(re_n))
        next_re_n = ptu.from_numpy(np.concatenate(next_re_n))
        ...
        for i in range(sample_num):
          ac_pred = ptu.from_numpy(self.actor.get_action(ptu.to_numpy(ob_no)))
          # next_ac_pred = ptu.from_numpy(self.actor.get_action(ptu.to_numpy(next_ob_no)))
          # diff += re_n + self.gamma * self.critic(torch.cat((next_ob_no, next_ac_pred, ob_no, ac_pred),-1))
          
          diff = self.critic(torch.cat((ob_no, ac_na, ob_no, ac_pred),-1))
        diff = ptu.to_numpy(diff) / sample_num
    ...
    def sample(self, batch_size):
        return self.replay_buffer.sample_recent_data(batch_size, False)
